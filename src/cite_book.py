...
class CiteBook(Gtk.Window):
	"""this class cites books"""
	...
	def __init__(self,style):
		...
		self.set_title(f"Enter information to cite book in {self.style} Style:")
		...
		self.format = ""
		self.create_citebook_interface()
		self.connect("destroy", self.close_window)
		self.show_all()
		
	def create_citebook_interface(self):
		"""create the gui for citing books"""		
		...
		self.entry_place = Gtk.Entry(xalign = 0)
		...
		self.box_9.pack_start(back_button,False,False,0)
		
		# go and copy button
		but_go = Gtk.Button(label = "Go!")
		self.box_9.pack_end(but_go,False,False,0)
		but_go.connect("clicked",self.butclicked)
		
		but_copy = Gtk.Button(label = "Copy")
		self.box_9.pack_end(but_copy,False,False,0)
	...
	def harvard_citation(self):
		...
		self.format += f" ({year}), {title.title()}, {place.title()}: {publisher.title()}"
